...
**主板（Motherboard / Mainboard）** 是整机的「背板」：所有核心部件都插在它上面，通过板内走线连到 CPU 和芯片组。学 CUDA 时不必会修主板，但知道**各插槽干什么、数据从哪条路走**，有助于理解 PCIe 拓扑、NUMA 和训练节点的 I/O 瓶颈。
...
| **PCIe 插槽** | 扩展槽，×16 / ×8 / ×4 等 | **GPU 显卡**、部分网卡、部分 NVMe 转接卡插这里；带宽受 CPU **lane 分配**限制 |
...
1. **GPU 插在哪个 PCIe 槽、连到哪个 CPU（NUMA 节点）**，会影响 `cudaMemcpy` 带宽——多路机上用 `nvidia-smi topo -m`、`numactl --hardware` 查看。  
